...
class TestBase:
	...
	async def create_and_fetch_single_attribute_test(self, endpoint, schema, attribute: str, value: Any,
	                                                 compare_method=None, additional_attributes=None):
		try:
	...
	async def setup(self):
		...
		res = await self.api(
			'POST', '/api/tenants/users/login',
			_body={'username': 'admin', 'password': '123'},
			_headers={'X-Tenant-ID': str(self.id_tenant)}
			)
		assert res.status_code == 200
	...
	async def api(self, _method: AnyStr, _endpoint: AnyStr, _body: Optional[Dict] = None, _params=None,
	              _headers=None) -> Response | Dict:
